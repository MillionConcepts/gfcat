...
from gPhoton import gAperture,PhotonPipe
import pandas as pd
import os
import shutil
...
def recalibrate(eclipse,band,
    ...
    if os.path.exists(os.path.dirname(photonfile)+'/No{band}'.format(band=band)):
        print('No {band} data for eclipse {eclipse}'.format(
    ...
    if not os.path.exists(photonfile):
        ...
        if ((obstype in ['AIS', 'NoData', 'GII', 'CAI', 'Unknown']) or
            ...
            if obstype in ['AIS']:
                pathlib.Path(os.path.dirname(photonfile)+'/{obstype}'.format(
                                                        obstype=obstype)).touch()
            if not retain:
                shutil.rmtree('{d}/e{e}/'.format(d=data_directory,e=eclipse))
            return
        if rawexpt<=600:
            pathlib.Path(os.path.dirname(photonfile)+'/{obstype}'.format(
                                                    obstype='LowExpt')).touch()
            if not retain:
                shutil.rmtree('{d}/e{e}/'.format(d=data_directory,e=eclipse))
            return
    ...
    if not raw6file:
        pathlib.Path(os.path.dirname(photonfile)+'/No{band}'.format(band=band)).touch()
        if not retain:
            shutil.rmtree('{d}/e{e}/'.format(d=data_directory,e=eclipse))
        return
    ...
    if not os.path.exists(photonfile):
        try:
            PhotonPipe.photonpipe(raw6file.split('.')[0][:-5],band,
                raw6file=raw6file,verbose=2)
        except:
            print('Error during data processing. (Probably a in the SSD calculation!)')
            pathlib.Path(os.path.dirname(photonfile)+'/No{band}'.format(band=band)).touch()
            pathlib.Path(os.path.dirname(photonfile)+'/SSDlinalg_ERROR'.format(band=band)).touch()
    ...
    if not os.path.exists(xcalfilename):
        ...
        try:
            ...
            print('Data file too big to be processed on this machine. Flagging for reprocessing.')
            pathlib.Path(os.path.dirname(photonfile)+'/No{band}'.format(band=band)).touch()
            pathlib.Path(os.path.dirname(photonfile)+'/TooBig'.format(band=band)).touch()
    else:
        print('{xcalfilename} already exists'.format(xcalfilename=xcalfilename))
