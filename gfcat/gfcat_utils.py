...
from gPhoton import curvetools as ct
from gPhoton import galextools as gt
from gPhoton import dbasetools as dt
from gPhoton import FileUtils as fu
from gPhoton import gQuery as gq
...
def calibrate_photons(photon_file, band):
    data = pd.read_hdf(photon_file.replace(".csv", ".h5"), "photons")

    image = pd.DataFrame()
    flat, _ = cal.flat(band)
    col, row = ct.xieta2colrow(np.array(data["xi"]), np.array(data["eta"]), band)
    ...
    flat = flat[
        np.array(image["col"], dtype="int16"), np.array(image["row"], dtype="int16")
    ]
